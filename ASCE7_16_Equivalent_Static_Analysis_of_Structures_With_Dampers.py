...
class Initials: # A class for calculating the initial analysis values
    ...
    def Any_B_value(any_beta):
        ...
        if any_beta<=0.02:
            any_B=0.8
            return any_B
        elif any_beta>0.02 and any_beta<1:
            any_B= round(((3.11*any_beta)+0.86),2)
            residual=((any_B*100)%10)/100
            if 0<=residual and residual<0.05:
                any_B-=residual
                return any_B
            elif 0.05<=residual and residual<=0.09:
                any_B+=(0.1-residual)
                return any_B
            else:
